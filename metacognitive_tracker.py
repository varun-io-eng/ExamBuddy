...
def render_daily_review_queue(user_id: str, dkt_tracker,
    ...
    for i, item in enumerate(queue, 1):
        ...
        with st.expander(
            ...
            if on_practice_click:
                # Key uses index + sanitized subject + topic to guarantee uniqueness
                # even if the same topic appears multiple times in the queue.
                # Avoid hash() — it is session-stable in CPython 3.3+ only with
                # PYTHONHASHSEED=0; on Render/production the seed is random,
                # so the same string can produce the same hash for two different
                # items in the same render pass, causing duplicate-key errors.
                _safe = lambda s: "".join(c if c.isalnum() else "_" for c in s)
                _btn_key = f"review_btn_{i}_{_safe(item['subject'])}_{_safe(item['topic'])}"
                if st.button(
                    f"🚀 Practice {item['topic']} Now",
                    key=_btn_key,
                    use_container_width=True,
